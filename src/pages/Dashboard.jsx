...
export default function Dashboard() {
    // 1. USESTATE: Mengelola filter pendapatan aktif
    const [incomeFilter, setIncomeFilter] = useState("monthly");
    const [incomeData, setIncomeData] = useState([]);
    const [isLoading, setIsLoading] = useState(false);

    // 2. USEREF: Melacak berapa kali komponen di-render tanpa memicu re-render ulang
    const renderCounter = useRef(0);
    renderCounter.current += 1;

    // Dataset awal default
    const chartDataDefault = [
        { month: "Jan", income: 40 }, { month: "Feb", income: 65 },
        { month: "Mar", income: 45 }, { month: "Apr", income: 90 },
        { month: "Mei", income: 55 }, { month: "Jun", income: 75 },
        { month: "Jul", income: 40 }, { month: "Agu", income: 85 },
        { month: "Sep", income: 60 }, { month: "Okt", income: 70 },
        { month: "Nov", income: 50 }, { month: "Des", income: 95 },
    ];

    // 3. USEEFFECT: Simulasi fetch data dummy setiap kali incomeFilter berubah
    useEffect(() => {
        setIsLoading(true);
        
        // Simulasi delay API backend selama 400ms
        const timer = setTimeout(() => {
            if (incomeFilter === "weekly") {
                // Modifikasi data acak untuk mingguan
                setIncomeData(chartDataDefault.map(d => ({ ...d, income: Math.max(10, d.income - 20) })));
            } else if (incomeFilter === "yearly") {
                // Modifikasi data acak untuk tahunan
                setIncomeData(chartDataDefault.map(d => ({ ...d, income: Math.min(100, d.income + 5) })));
            } else {
                setIncomeData(chartDataDefault);
            }
            setIsLoading(false);
        }, 400);
    ...
    const newMembers = [
        { name: "Aiden Max", email: "aiden@zeus.com", status: "Active", avatar: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=100&h=100&q=80" },
        { name: "Roselle Ehrman", email: "roselle@gmail.com", status: "Active", avatar: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=100&h=100&q=80" },
        { name: "Jone Smith", email: "jone@gmail.com", status: "Pending", avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=100&h=100&q=80" }
    ];

    return (
        <div className="space-y-8 p-2 font-dmsans text-white">
            {/* Tampilan Log Render Counter dari useRef */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard label="Monthly Income" amount="Rp 45.200.000" growth="+12.5%" icon={<MdAccountBalanceWallet className="text-white text-2xl" />} iconBg="bg-[#1cc0a0]" />
                <StatCard label="New Members" amount="124" growth="+8%" icon={<MdPeople className="text-white text-2xl" />} iconBg="bg-[#56ccf2]" />
            ...
            <div className="grid grid-cols-12 gap-8">
                <div className="col-span-12 lg:col-span-8 space-y-8">
                    ...
                    <div className="bg-[#20223b] rounded-3xl p-7 border border-gray-800 shadow-xl">
                        ...
                        {/* Area Grafik Utama */}
                        <div className={`w-full h-56 flex gap-4 transition-opacity duration-200 ${isLoading ? 'opacity-40' : 'opacity-100'}`}>
                            ...
                            <div className="flex-1 flex flex-col justify-between h-full">
                                <div className="flex-1 flex items-end justify-between gap-2 px-2 pt-2 border-b border-gray-800/60 relative">
                                    ...
                                    {/* Merender data responsif dari state incomeData */}
